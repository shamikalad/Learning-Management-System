                ...
                <div class="card" >
                    <div class="card-body">
                        ...
                        <%
                        	...
                        	Pro_Model pm3=new Pro_Model(Connection_Jdbc.getConn());
                        	student_registration sr=pm3.studentrecordfetch(stud_id);
                        %>
                        
                        <form class="row g-3" action="studentupdateinsert" method="post">
                            
                            <input type="hidden" name="studid" value=<%=sr.getStud_id()%>>
                            <input type="hidden" name="enqid" value=<%=sr.getEnq_id() %>>
                            <div class="col-12">
                                <label for="name" class="form-label">Name</label>
                                <input type="text" class="form-control" id="name" name="name" value="<%=sr.getName()%>"required>
                            </div>
                           <%--  <div class="col-12">
                                <label for="address" class="form-label">Address</label>
                                <input type="text" class="form-control" id="address" name="address" value="<%=sr.get%>"required>
                            </div> --%>
                            <div class="col-12">
                                <label for="email" class="form-label">Email</label>
                                <input type="email" class="form-control" id="email" name="email" value="<%=sr.getEmail()%>>" required>
                            </div>
                            <div class="col-12">
                                <label for="mobile" class="form-label">Mobile</label>
                                <input type="text" class="form-control" id="mobile" name="mobile" value="<%=sr.getMob()%>" required>
                            </div>
	                    ...
	                    <select class="form-select" aria-label="Default select example" name="coursenm" id="floatingSelect">
	                      ...
	                      try{
	                    	  ...
	                    	  {
	                      %>
	                      <option value="<%=c.getCourse_id()%>" <%=sr.getCourse_id() ==c.getCourse_id() ? "selected" : "" %>><%=c.getCourse_name() %></option>
	                     <% 
	                    	  }
	                      }catch(Exception e)
	                      {
